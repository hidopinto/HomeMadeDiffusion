---
name: check-shapes
description: Runs pytest shape tests and provides a deep-dive analysis of tensor dimension mismatches.
---
...
## Execution
1. Run the shape tests:
   ! pytest tests/ -v -k "shape" --tb=short
2. If tests fail, capture the traceback.
...
- Check `config.yaml` to see if `is_video` is True — if so, ensure the `SinCosPosEmbed3D` is receiving 5D tensors `(B, C, T, H, W)` and `patch_size` has 3 elements.
